...
struct RadialFunctionG
{
  ...
  RadialFunctionG();
  void init(int l, int nSamples, double dG, const char* filename,
            double scale =
              1.0); //!< read and initialize from an ascii file (DFT PSP format)
  void init(int l, const std::vector<double>& samples,
            double dG); //!< initialize from an array of samples in memory
  void set(
    const std::vector<double>& coeff,
    double dGInv); //!< set the coefficients (and update the GPU versions etc.)
  void updateGmax(int l, int nSamples); //!< if created from a RadialFunctionR,
                                        //!increase nCoeff if necessary (call
                                        //!when lattice is modified)
  ...
  __hostanddev__ double operator()(double G) const
  {
    ...
    if (Gindex >= nCoeff - 5)
      ...
      return QuinticSpline::value(getCoeff(), Gindex);
  }

  RadialFunctionR*
    rFunc; //!< copy of the real-space radial version (if created from one)
...
struct RadialFunctionR
{
  ...
  RadialFunctionR(const std::vector<double>& r,
                  double dlogr); //!< initialize logarithmic grid
  void set(std::vector<double> r,
           std::vector<double> dr); //!< update the sample locations, weights

  //! Compute optimum dr given r (must be in ascending order) which will be
  //! exact
  //! for the integration of cubic splines with natural boundary conditions.
  //! It is recommended that r[0]=0 or r[0]<<r[1], but it'll work fine even
  //! otherwise.
  void initWeights();
