...
RadialFunctionG::RadialFunctionG()
  : dGinv(0)
  , nCoeff(0)
  ,
#ifdef GPU_ENABLED
...
void
RadialFunctionG::init(int l, int nSamples, double dG, const char* filename,
                      double scale)
{
  std::vector<double> samples(nSamples);
  FILE* fp = fopen(filename, "r");
  if (!fp)
    die("Could not open radial function file '%s' for reading.\n", filename);
...
void
RadialFunctionG::init(int l, const std::vector<double>& samples, double dG)
{
  set(QuinticSpline::getCoeff(samples, l % 2 == 1), 1. / dG);
}

void
RadialFunctionG::set(const std::vector<double>& coeff, double dGinv)
{
  this->coeff = coeff;
  this->dGinv = dGinv;
  nCoeff = coeff.size();
  ...
  cudaMemcpy(coeffGpu, coeff.data(), sizeof(double) * nCoeff,
...
void
RadialFunctionG::updateGmax(int l, int nSamples)
{
  if (!rFunc)
    return; // don't have the information necessary to update
  if (nSamples + 4 <= nCoeff)
    return; // already have enough samples
  rFunc->transform(l, 1. / dGinv, nSamples, *this);
}

void
RadialFunctionG::free(bool rFuncDelete)
{
  if (rFunc && rFuncDelete)
    delete rFunc;
#ifdef GPU_ENABLED
  if (nCoeff)
    cudaFree(coeffGpu);
...
RadialFunctionR::RadialFunctionR(int nSamples)
  : r(nSamples)
  , dr(nSamples)
  , f(nSamples)
{
...
void
RadialFunctionR::set(std::vector<double> r, std::vector<double> dr)
{
...
void
RadialFunctionR_initWeights_sub(int jStart, int jStop, int nSamples,
                                const double* r, double* dr)
{
  for (int j = jStart; j < jStop;
       j++) { // Calculate cubic spline with natural B.C. that is 1 at j'th
              // point and 0 at all others:
    ...
    std::vector<double> y(nSamples, 0.);
    y[j] = 1; // values of spline y(r) at nodes
    for (int i = 0; i < nSamples - 1;
         i++) { // Contribution from (r[i],r[i+1]) interval:
      double hInv = 1. / (r[i + 1] - r[i]);
      double rhs = 3 * hInv * hInv * (y[i + 1] - y[i]);
      b[i] += 2 * hInv;
      c[i] += hInv;
      d[i] += rhs; // to i'th equation
      a[i + 1] += hInv;
      b[i + 1] += 2 * hInv;
      d[i + 1] += rhs; // to i+1'th equation
    }
    ...
    // Integrate spline over entire domain to get quadrature weight for current
    // node:
    double r0 = r[0], y0 = y[0], yp0 = yp[0];
    double wj = (1. / 3) * (r0 * r0 * r0) *
                (y0 - 0.25 * r0 * yp0); // contribution from r=0 till first node
...
void
RadialFunctionR::initWeights()
{
  static StopWatch watch("initWeights");
  watch.start();
  ...
  if (!r[0])
    r[0] = 1e-6 *
           r[1]; // avoid division by 0 (will make no difference at double prec)
  threadLaunch(RadialFunctionR_initWeights_sub, nSamples, nSamples, r.data(),
               dr.data());
  watch.stop();
}
...
double
RadialFunctionR::transform(int l, double G) const
{ // Simpson's 1/3 rule on the log-grid:
  double sum = 0.0;
  int Nhlf = (f.size() - 1) / 2; // half the sample points
  for (int i = 0; i <= 2 * Nhlf; i++) {
    sum += f[i] *
           bessel_jl(l, G * r[i]) // sample value for spherical bessel transform
           * (dr[i] * r[i] * r[i]) // weight for radial spherical integration
           * ((i == 0 || i == 2 * Nhlf)
                ? 1
                : 2 * ((i % 2) + 1)); // simpson integration factor
  }
  return (4 * M_PI / 3) * sum;
}

static void
RadialFunction_transform_sub(int iGstart, int iGstop, int l, double dG,
                             const RadialFunctionR* rFunc, double* fTilde)
{
...
void
RadialFunctionR::transform(int l, double dG, int nGrid,
                           RadialFunctionG& func) const
{
  static StopWatch watch("RadialFunctionR::transform");
  watch.start();
