...
//! The periodicity of both grids are broken on their Wigner-Seitz (WS) cells
//! centered at the origin,
//! and the values in the input WS outside the output WS are truncated, while
//! the region in the
...
// Compute the kinetic energy for a blip orbital phi (and set max local KE and
// location in unit cell)
double Tblip(const complexScalarField& phi, double* tMax = 0, int* i0max = 0,
             int* i1max = 0, int* i2max = 0);
