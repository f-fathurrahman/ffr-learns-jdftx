...
bool
diagMatrix::isScalar(double absTol, double relTol) const
{
  double mean = 0.0;
  for (double d : *this)
    mean += d;
...
void
diagMatrix::set(int iStart, int iStop, const diagMatrix& m)
{
  assert(iStart >= 0 && iStart < nRows());
  assert(iStop > iStart && iStop <= nRows());
  int iDelta = iStop - iStart;
...
diagMatrix::operator()(int iStart, int iStep, int iStop) const
{
  ...
  diagMatrix ret(iDelta);
  for (int i = 0; i < iDelta; i++)
    ret[i] = at(i * iStep + iStart);
  return ret;
}
void
diagMatrix::set(int iStart, int iStep, int iStop, const diagMatrix& m)
{
  assert(iStart >= 0 && iStart < nRows());
  assert(iStop > iStart && iStop <= nRows());
  assert(iStep > 0);
  ...
  assert(iDelta == m.nRows());
  for (int i = 0; i < iDelta; i++)
    at(i * iStep + iStart) = m[i];
}

void
diagMatrix::scan(FILE* fp)
{
  for (double& d : *this)
    fscanf(fp, "%lg", &d);
}
...
void
diagMatrix::print(FILE* fp, const char* fmt) const
{
  for (double d : *this)
    fprintf(fp, fmt, d);
  fprintf(fp, "\n");
...
void
diagMatrix::send(int dest, int tag) const
{
  assert(mpiUtil->nProcesses() > 1);
  mpiUtil->send(data(), size(), dest, tag);
}
void
diagMatrix::recv(int src, int tag)
{
  assert(mpiUtil->nProcesses() > 1);
  mpiUtil->recv(data(), size(), src, tag);
}
void
diagMatrix::bcast(int root)
{
  if (mpiUtil->nProcesses() > 1)
    mpiUtil->bcast(data(), size(), root);
}
void
diagMatrix::allReduce(MPIUtil::ReduceOp op, bool safeMode)
{
  if (mpiUtil->nProcesses() > 1)
...
// Initialization
void
matrix::init(int nrows, int ncols, bool onGpu)
{
  nr = nrows;
  nc = ncols;

  if (nr * nc > 0)
    memInit("matrix", nr * nc, onGpu);
}
// Reshaping
void
matrix::reshape(int nrows, int ncols)
{
  assert(nrows >= 0);
  assert(ncols >= 0);
...
matrix::operator()(int i, int j) const
{
  ...
  if (isOnGpu()) {
    complex ret;
#ifdef GPU_ENABLED
    cudaMemcpy(&ret, dataGpu() + index(i, j), sizeof(complex),
...
#ifdef GPU_ENABLED
void matrixSubGet_gpu(int nr, int iStart, int iStep, int iDelta, int jStart,
                      int jStep, int jDelta, const complex* in,
                      complex* out); // implemented in operators.cu
#endif
matrix
matrix::operator()(int iStart, int iStep, int iStop, int jStart, int jStep,
                   int jStop) const
{
  if (iStart == 0 && iStep == 1 && iStop == nr && jStart == 0 && jStep == 1 &&
      jStop == nc)
  ...
  int iDelta = ceildiv(iStop - iStart, iStep),
      jDelta = ceildiv(jStop - jStart, jStep);
  matrix ret(iDelta, jDelta, isGpuEnabled());
  complex* retData = ret.dataPref();
  const complex* thisData = this->dataPref();
#ifdef GPU_ENABLED
  matrixSubGet_gpu(nr, iStart, iStep, iDelta, jStart, jStep, jDelta, thisData,
                   retData);
#else
  for (int i = 0; i < iDelta; i++)
    for (int j = 0; j < jDelta; j++)
      retData[ret.index(i, j)] =
        thisData[this->index(i * iStep + iStart, j * jStep + jStart)];
#endif
  return ret;
}

void
matrix::set(int i, int j, complex m)
{
  assert(i < nr and i >= 0);
  assert(j < nc and j >= 0);
  if (isOnGpu()) {
#ifdef GPU_ENABLED
    cudaMemcpy(dataGpu() + index(i, j), &m, sizeof(complex),
...
#ifdef GPU_ENABLED
void matrixSubSet_gpu(int nr, int iStart, int iStep, int iDelta, int jStart,
                      int jStep, int jDelta, const complex* in,
                      complex* out); // implemented in operators.cu
#endif
void
matrix::set(int iStart, int iStep, int iStop, int jStart, int jStep, int jStop,
            const matrix& m)
{
  assert(iStart >= 0 && iStart < nr);
  assert(iStop > iStart && iStop <= nr);
  ...
  const complex* mData = m.dataPref();
  complex* thisData = this->dataPref();
#ifdef GPU_ENABLED
  matrixSubSet_gpu(nr, iStart, iStep, iDelta, jStart, jStep, jDelta, mData,
                   thisData);
#else
  for (int i = 0; i < iDelta; i++)
    for (int j = 0; j < jDelta; j++)
...
void
matrix::scan(FILE* fp, const char* fmt)
{
...
void
matrix::print(FILE* fp, const char* fmt) const
{
...
void
matrix::print_real(FILE* fp, const char* fmt) const
{
  const complex* thisData = this->data();
  for (int i = 0; i < nRows(); i++) {
    for (int j = 0; j < nCols(); j++)
...
void zheevr_(char* JOBZ, char* RANGE, char* UPLO, int* N, complex* A, int* LDA,
             ...
             double* W, complex* Z, int* LDZ, int* ISUPPZ, complex* WORK,
             int* LWORK, double* RWORK, int* LRWORK, int* IWORK, int* LIWORK,
             int* INFO);
}
void
matrix::diagonalize(matrix& evecs, diagMatrix& eigs) const
{
  ...
  if (hermErr > 1e-10) {
    logPrintf(
      "Relative hermiticity error of %le (>1e-10) encountered in diagonalize\n",
      hermErr);
    stackTraceExit(1);
  }
  ...
  double eigMin = 0.,
         eigMax = 0.; // eigenvalue range (not used for range-type 'A')
  int indexMin = 0,
      indexMax = 0; // eignevalue index range (not used for range-type 'A')
  double absTol = 0.;
  int nEigsFound;
  eigs.resize(N);
  evecs.init(N, N);
  ...
  int lwork = (64 + 1) * N;
  std::vector<complex> work(lwork); // Magic number 64 obtained by running
                                    // ILAENV as suggested in doc of zheevr (and
                                    // taking the max over all N)
  int lrwork = 24 * N;
  std::vector<double> rwork(lrwork); // from doc of zheevr
  int liwork = 10 * N;
  std::vector<int> iwork(liwork); // from doc of zheevr
  int info = 0;
  zheevr_(&jobz, &range, &uplo, &N, A.data(), &N, &eigMin, &eigMax, &indexMin,
          &indexMax, &absTol, &nEigsFound, eigs.data(), evecs.data(), &N,
          iSuppz.data(), work.data(), &lwork, rwork.data(), &lrwork,
          iwork.data(), &liwork, &info);
  if (info < 0) {
    logPrintf("Argument# %d to LAPACK eigenvalue routine ZHEEVR is invalid.\n",
              -info);
    stackTraceExit(1);
  }
  if (info > 0) {
    logPrintf("Error code %d in LAPACK eigenvalue routine ZHEEVR.\n", info);
    stackTraceExit(1);
  }
  watch.stop();
}
...
extern "C" {
void zgeev_(char* JOBVL, char* JOBVR, int* N, complex* A, int* LDA, complex* W,
            complex* VL, int* LDVL, complex* VR, int* LDVR, complex* WORK,
            int* LWORK, double* RWORK, int* INFO);
}
void
matrix::diagonalize(matrix& levecs, std::vector<complex>& eigs,
                    matrix& revecs) const
{
  ...
  int info = 0;
  zgeev_(&jobz, &jobz, &N, A.data(), &N, eigs.data(), levecs.data(), &N,
         revecs.data(), &N, work.data(), &lwork, rwork.data(), &info);
  if (info < 0) {
    logPrintf("Argument# %d to LAPACK eigenvalue routine ZGEEV is invalid.\n",
              -info);
    stackTraceExit(1);
  }
  if (info > 0) {
    logPrintf("Error code %d in LAPACK eigenvalue routine ZGEEV.\n", info);
    stackTraceExit(1);
  }
  watch.stop();
}
...
extern "C" {
void zgesdd_(char* JOBZ, int* M, int* N, complex* A, int* LDA, double* S,
             complex* U, int* LDU, complex* VT, int* LDVT, complex* WORK,
...
void
matrix::svd(matrix& U, diagMatrix& S, matrix& Vdag) const
{
  ...
  std::vector<double> rwork(S.nRows() *
                            std::max(5 * S.nRows() + 7, 2 * (M + N) + 1));
  ...
  if (info > 0) // convergence failure; try the slower stabler version
  {
    int info = 0;
    matrix A = *this; // destructible copy
    zgesvd_(&jobz, &jobz, &M, &N, A.data(), &M, S.data(), U.data(), &M,
            Vdag.data(), &N, work.data(), &lwork, rwork.data(), &info);
    if (info < 0) {
      logPrintf("Argument# %d to LAPACK SVD routine ZGESVD is invalid.\n",
                -info);
      stackTraceExit(1);
    }
    if (info > 0) {
      logPrintf("Error code %d in LAPACK SVD routine ZGESVD.\n", info);
      stackTraceExit(1);
    }
  }
  if (info < 0) {
    logPrintf("Argument# %d to LAPACK SVD routine ZGESDD is invalid.\n", -info);
    stackTraceExit(1);
  }
  watch.stop();
}
...
matrixScaledTransOp::operator matrix() const
{
  if (op == CblasNoTrans)
    ...
    matrix ret(mat.nCols(), mat.nRows());
...
// Hermitian adjoint
matrixScaledTransOp
dagger(const scaled<matrix>& A)
{
  return matrixScaledTransOp(A.data, A.scale, CblasConjTrans);
}
matrix
dagger_symmetrize(const scaled<matrix>& A)
{
  return 0.5 * (dagger(A) + A);
}
// Transpose
matrixScaledTransOp
transpose(const scaled<matrix>& A)
{
  return matrixScaledTransOp(A.data, A.scale, CblasTrans);
}
matrix
transpose_symmetrize(const scaled<matrix>& A)
{
...
matrix operator*(const matrixScaledTransOp& m1st,
                 const matrixScaledTransOp& m2st)
{
  ...
  callPref(eblas_zgemm)(m1st.op, m2st.op, ret.nRows(), ret.nCols(),
                        m1st.nCols(), scaleFac, m1.dataPref(), m1.nRows(),
                        m2.dataPref(), m2.nRows(), 0.0, ret.dataPref(),
                        ret.nRows());
  return ret;
}
...
#ifdef GPU_ENABLED
  double* dData;
  cudaMalloc(&dData, d.nRows() * sizeof(double));
  cudaMemcpy(dData, &d[0], d.nRows() * sizeof(double), cudaMemcpyHostToDevice);
  gpuErrorCheck();
#else
  const double* dData = &d[0];
#endif
  // Elementwise-multiply each column by the scale factors (better contiguous
  // access pattern than row-wise)
  for (int j = 0; j < ret.nCols(); j++)
    callPref(eblas_zmuld)(ret.nRows(), dData, 1,
                          ret.dataPref() + ret.index(0, j), 1);
#ifdef GPU_ENABLED
  cudaFree(dData);
#endif
  return ret;
...
matrix operator*(const matrix& m, const diagMatrix& d)
{
  ...
  // Scale each column:
  for (int j = 0; j < ret.nCols(); j++)
    callPref(eblas_zdscal)(ret.nRows(), d[j], ret.dataPref() + ret.index(0, j),
                           1);
  return ret;
}
...
diagMatrix operator*(const diagMatrix& d1, const diagMatrix& d2)
{
  ...
  for (int i = 0; i < ret.nRows(); i++)
    ret[i] *= d2[i]; // elementwise multiply
  return ret;
}

void
axpy(double alpha, const diagMatrix& x, matrix& y)
{
  assert(x.nRows() == y.nRows());
  assert(x.nCols() == y.nCols());
  complex* yData = y.data();
  for (int i = 0; i < y.nRows(); i++)
    yData[y.index(i, i)] += alpha * x[i];
}

void
axpy(double alpha, const diagMatrix& x, diagMatrix& y)
{
  assert(x.nRows() == y.nRows());
  for (int i = 0; i < y.nRows(); i++)
    y[i] += alpha * x[i];
...
diagMatrix
clone(const diagMatrix& x)
{
  return x;
}
matrix
clone(const matrix& x)
{
  return x;
}

double
dot(const diagMatrix& x, const diagMatrix& y)
{
...
double
dot(const matrix& x, const matrix& y)
{
  return dotc(x, y).real();
}

void
randomize(diagMatrix& x)
{
  for (size_t i = 0; i < x.size(); i++)
    x[i] = Random::normal();
}
...
void zgetri_(int* N, complex* A, int* LDA, int* IPIV, complex* WORK, int* LWORK,
             int* INFO);
void zposv_(char* UPLO, int* N, int* NRHS, complex* A, int* LDA, complex* B,
            int* LDB, int* INFO);
}
...
inv(const matrix& A)
{
  ...
  if (info < 0) {
    logPrintf(
      "Argument# %d to LAPACK LU decomposition routine ZGETRF is invalid.\n",
      -info);
    stackTraceExit(1);
  }
  if (info > 0) {
    logPrintf("LAPACK LU decomposition routine ZGETRF found input matrix to be "
              "singular at the %d'th step.\n",
              info);
    stackTraceExit(1);
  }
  ...
  if (info < 0) {
    logPrintf(
      "Argument# %d to LAPACK matrix inversion routine ZGETRI is invalid.\n",
      -info);
    stackTraceExit(1);
  }
  if (info > 0) {
    logPrintf("LAPACK matrix inversion routine ZGETRI found input matrix to be "
              "singular at the %d'th step.\n",
              info);
    stackTraceExit(1);
  }
...
matrix
invApply(const matrix& A, const matrix& b)
{
  ...
  if (hermErr > 1e-10) {
    logPrintf(
      "Relative hermiticity error of %le (>1e-10) encountered in invApply\n",
      hermErr);
    stackTraceExit(1);
  }
  ...
  if (info < 0) {
    logPrintf("Argument# %d to LAPACK linear solve routine ZPOSV is invalid.\n",
              -info);
    stackTraceExit(1);
  }
  if (info > 0) {
    logPrintf("Matrix not positive-definite at leading minor# %d in LAPACK "
              "linear solve routine ZPOSV.\n",
              info);
    stackTraceExit(1);
  }
  watch.stop();
...
matrix
pow(const matrix& A, double exponent, matrix* Aevecs, diagMatrix* Aeigs)
{
  MATRIX_FUNC(if (exponent < 0. && eigs[i] <= 0.0) {
    logPrintf("Eigenvalue# %d is non-positive (%le) in pow (exponent %lg)\n", i,
              eigs[i], exponent);
    stackTraceExit(1);
  } else if (exponent >= 0. && eigs[i] < 0.0) {
    logPrintf("WARNING: Eigenvalue# %d is negative (%le) in pow (exponent "
              "%lg); zeroing it out.\n",
              i, eigs[i], exponent);
    eigs[i] = 0.;
...
matrix
invsqrt(const matrix& A, matrix* Aevecs, diagMatrix* Aeigs)
{
  return pow(A, -0.5, Aevecs, Aeigs);
}

// Compute cis(A) = exp(iota A) and optionally the eigensystem of A (if
// non-null)
matrix
cis(const matrix& A, matrix* Aevecs, diagMatrix* Aeigs)
{
  MATRIX_FUNC(eigOut[i] = cis(eigs[i]);)
}
...
matrix
cisInv(const matrix& A, matrix* Bevecs, diagMatrix* Beigs)
{ // Make sure A is unitary:
  assert(A.nRows() == A.nCols());
  assert(nrm2(A * dagger(A) - eye(A.nRows())) < 1e-10 * sqrt(A.nData()));
  // Diagonalize:
  matrix Alevecs, Arevecs;
  std::vector<complex> Aeigs;
...
matrix
sqrt_grad(const matrix& gradIn, const matrix& Aevecs, const diagMatrix& Aeigs)
{
  MATRIX_FUNC_GRAD(elem /= (sqrt(Aeigs[i]) + sqrt(Aeigs[j]));)
}
// Return gradient w.r.t A given gradient w.r.t cis(A) and A's eigensystem
matrix
cis_grad(const matrix& gradIn, const matrix& Aevecs, const diagMatrix& Aeigs)
{
  MATRIX_FUNC_GRAD(double x = Aeigs[j] - Aeigs[i];
                   elem *= fabs(x) < 1.0e-13 ? complex(-0.5 * x, 1)
                                             : (cis(x) - 1.0) / x;)
}
...
tiledBlockMatrix::tiledBlockMatrix(const matrix& mBlock, int nBlocks,
                                   const std::vector<complex>* phaseArr)
  : mBlock(mBlock)
  , nBlocks(nBlocks)
  , phaseArr(phaseArr)
{
  if (phaseArr)
    assert(nBlocks == int(phaseArr->size()));
}
...
matrix tiledBlockMatrix::operator*(const matrix& other) const
{
  ...
  for (int iBlock = 0; iBlock < nBlocks; iBlock++) {
    ...
    callPref(eblas_zgemm)(
      CblasNoTrans, CblasNoTrans, mBlock.nRows(), other.nCols(), mBlock.nCols(),
      phase, mBlock.dataPref(), mBlock.nRows(), other.dataPref() + offs,
      other.nRows(), 0.0, result.dataPref() + offs, result.nRows());
  }
  return result;
}
...
complex
matrix::getElement(vector3<int> index, GridInfo& gInfo)
{
  matrix result(1, 1); // use a 1x1 matrix to store the result locally (CPU/GPU)
  callPref(eblas_copy)(result.dataPref(), dataPref() + gInfo.fullRindex(index),
                       1);
