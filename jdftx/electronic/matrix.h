...
public:
  ...
  bool isScalar(double absTol = 1e-14, double relTol = 1e-14)
    const; //!< Check if all entries are identical within threshold
  ...
  // Splicing operations:
  diagMatrix operator()(int iStart, int iStop)
    const; //! get submatrix of elements (iStart \<= i \< iStop)
  diagMatrix operator()(int iStart, int iStep,
                        int iStop) const; //! get submatrix of elements (iStart
                                          //! \<= i \< iStop) with arbitrary
                                          //! increments
  void set(int iStart, int iStop, const diagMatrix& m); //! set submatrix to m
  void set(int iStart, int iStep, int iStop,
           const diagMatrix& m); //! set submatrix to m at arbitrary increments

  void scan(FILE* fp); //!< read (ascii) from stream
  void print(FILE* fp,
             const char* fmt = "%lg\t") const; //!< print (ascii) to stream

  // Inter-process communication:
  void send(int dest, int tag = 0) const; // send to another process
  void recv(int src, int tag = 0);        // receive from another process
  void bcast(int root = 0); // synchronize across processes (using value on
                            // specified root process)
  void allReduce(MPIUtil::ReduceOp op,
                 bool safeMode = false); // apply all-to-all reduction (see
                                         // MPIUtil::allReduce)
};
...
public:
  ...
  void init(int nrows, int ncols,
            bool onGpu = false);      //!< called by the constructors
  void reshape(int nrows, int ncols); //!< change the dimensions without
                                      //!altering the data (zero dimensions are
                                      //!filled in to match size)
  ...
  matrix(
    const std::vector<complex>&);    //!< convert from a complex diagonal matrix
  ...
  {
    return (*this)(iStart, 1, iStop, jStart, 1, jStop);
  }

  //! get submatrix of elements (iStart \<= i \< iStop, jStart \<= j \< jStop)
  //! with arbitrary increments
  ...
  void set(int iStart, int iStep, int iStop, int jStart, int jStep, int jStop,
           const matrix& m);

  void scan(FILE* fp,
            const char* fmt = "%lg%+lgi"); //!< read (ascii) from stream
  void scan_real(FILE* fp); //!< read (ascii) real parts from stream, setting
                            //!imaginary parts to 0
  void print(FILE* fp,
             const char* fmt = "%lg%+lgi\t") const; //!< print (ascii) to stream
  void print_real(FILE* fp, const char* fmt = "%lg\t")
    const; //!< print (ascii) real parts to stream

  void diagonalize(matrix& evecs,
                   diagMatrix& eigs) const; //!< diagonalize a hermitian matrix
  void diagonalize(matrix& levecs, std::vector<complex>& eigs,
                   matrix& revecs) const; //!< diagonalize an arbitrary matrix
  void svd(matrix& U, diagMatrix& S,
           matrix& Vdag) const; //!< singular value decomposition (for
                                //!dimensions of this: MxN, on output U: MxM, S:
                                //!min(M,N), Vdag: NxN)

  //---------octave-like slicing operators on scalar fields converted to
  //Nx*Ny*Nz x 1 matrices --------------
...
struct matrixScaledTransOp
{
  ...
  {
    return (op == CblasConjTrans) ? a.conj() : a;
  } //!< return conjugate if op requires it
  int index(int i, int j) const
  {
    return op == CblasNoTrans ? mat.index(i, j) : mat.index(j, i);
  } //!< Index into data() with possible transpose

  //! Create from a matrix with an optional scale and op:
  matrixScaledTransOp(const matrix& mat, double scale = 1.0,
  ...
  matrixScaledTransOp(const scaled<matrix>& smat,
                      CBLAS_TRANSPOSE op = CblasNoTrans)
    : mat(smat.data)
    , scale(smat.scale)
    , op(op)
  ...
  {
    return matrixScaledTransOp(mat, scale * s, op);
  }
  friend matrixScaledTransOp operator*(double s, const matrixScaledTransOp& A)
  {
    return A * s;
  }
};
matrix conj(
  const scaled<matrix>& A); //!< return element-wise complex conjugate of A
matrixScaledTransOp dagger(
  const scaled<matrix>& A); //!< return hermitian adjoint of A
matrixScaledTransOp transpose(
  const scaled<matrix>& A); //!< return transpose of A
matrix dagger_symmetrize(
  const scaled<matrix>& A); //! return adjoint symmetric part: (A + Adag)/2
matrix transpose_symmetrize(
  const scaled<matrix>& A); //! return transpose symmetric part: (A + AT)/2
...
inline scaled<matrix> operator*(double s, const matrix& m)
{
  return scaled<matrix>(m, s);
}
inline scaled<matrix> operator*(const matrix& m, double s)
{
  return scaled<matrix>(m, s);
}
inline scaled<matrix>
operator-(const matrix& m)
{
  return scaled<matrix>(m, -1);
}
inline matrix&
operator*=(matrix& m, complex s)
{
  scale(s, m);
  return m;
}
inline matrix operator*(complex s, const matrix& m)
{
  matrix sm(m);
  sm *= s;
  return sm;
}
inline matrix operator*(const matrix& m, complex s)
{
  matrix sm(m);
  sm *= s;
  return sm;
}
inline diagMatrix operator*(double s, const diagMatrix& m)
{
  diagMatrix ret(m);
  return ret *= s;
}
inline diagMatrix operator*(const diagMatrix& m, double s)
{
  diagMatrix ret(m);
  return ret *= s;
}
inline diagMatrix
operator-(const diagMatrix& m)
{
  return m * (-1);
}
...
inline matrix&
operator+=(matrix& m, const diagMatrix& d)
{
  if (m)
    axpy(1., d, m);
  else
...
inline matrix
operator+(const matrix& m, const diagMatrix& d)
{
  matrix ret(m);
  ret += d;
  return ret;
...
inline matrix
operator+(const diagMatrix& d, const matrix& m)
{
  matrix ret(m);
  ret += d;
  return ret;
...
inline diagMatrix&
operator+=(diagMatrix& d1, const diagMatrix& d2)
{
  axpy(1., d2, d1);
  return d1;
}
...
inline diagMatrix
operator+(const diagMatrix& d1, const diagMatrix& d2)
{
  diagMatrix ret(d1);
  return ret += d2;
}
...
matrix invApply(const matrix& A, const matrix& b); // return inv(A) * b (A must
                                                   // be hermitian,
                                                   // positive-definite)
...
//! If skipZeros is true, skips diagonal entries in the diagonal of the LU
//! decomposition that are below a tolerance
...
matrix pow(const matrix& A, double exponent, matrix* Aevecs = 0,
           diagMatrix* Aeigs = 0);
...
//! Inverse function of cis: find matrix B such that A = exp(iota B). A must be
//! unitary.
//! Optionally retrieve the eigensystem of the result (note the real eigenvalues
//! are of the input, not the result)
...
matrix sqrt_grad(const matrix& grad_sqrtA, const matrix& Aevecs,
                 const diagMatrix& Aeigs);

//! Return gradient w.r.t A given gradient w.r.t cis(A) and A's eigensystem
matrix cis_grad(const matrix& grad_cisA, const matrix& Aevecs,
                const diagMatrix& Aeigs);
...
diagMatrix diag(
  const matrix& m);    //!< obtain the real diagonal part of a hermitian matrix
diagMatrix eye(int N); //!< identity
matrix zeroes(int nRows, int nCols); //!< a dense-matrix of zeroes

//! A block matrix formed by repeating (tiling) a dense matrix along the
//! diagonal
...
public:
  tiledBlockMatrix(const matrix& mBlock, int nBlocks,
                   const std::vector<complex>* phaseArr = 0);

  matrix operator*(const matrix&)
    const; //!< multiply block matrix by dense matrix
};
